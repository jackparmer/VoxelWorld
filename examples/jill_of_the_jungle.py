...
# Create a voxel matrix with Perlin noise to simulate jungle density
...
for x in range(size):
    for y in range(size):
        for z in range(size):
            noise_value = pnoise3(x / scale, y / scale, z / scale, octaves=4)
            if noise_value > threshold or (z < size / 2 and np.random.rand() > 0.95):  # More dense and random lower levels
                voxel_matrix[x, y, z] = 1

# Create a color matrix with lush jungle colors
    ...
    for y in range(size):
        for z in range(size):
            if voxel_matrix[x, y, z] == 1:
                if z > size / 2:
                    r = int((np.sin(x / 5.0) + 1) * 30 + 30)  # Darker green for higher levels
                    g = int((np.cos(y / 5.0) + 1) * 120 + 100)  # Lush green
                    b = int((np.sin(z / 5.0) + 1) * 30 + 30)  # Darker green
                else:
                    r = int((np.sin(x / 5.0) + 1) * 60 + 50)  # Brownish color for trunks/soil
                    g = int((np.cos(y / 5.0) + 1) * 60 + 30)  # Dark green/brown
                    b = int((np.sin(z / 5.0) + 1) * 40 + 20)  # Dark green/brown
                color_matrix[x, y, z] = (r, g, b)

# Initialize VoxelWorld with the voxel and color matrices
world = VoxelWorld(voxel_matrix, theme_name='Mint', resolution=10, zoom=2.0, show_light_source=False, dark_bg=True, color_matrix=color_matrix)
...
def create_rotating_frames(world, steps=36):
    ...
    for i in range(steps):
        ...
        byte_stream = VoxelWorld.Animations.create_voxel_img(rotated_voxel_matrix, 'Mint', resolution=10, viewing_angle=(45, 35), zoom=2.0, show_light_source=False, dark_bg=True, color_matrix=rotated_color_matrix)
